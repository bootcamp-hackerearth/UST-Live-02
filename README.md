...
On every push to the `deploy` branch, GitHub Actions (`.github/workflows/deploy.yml`) performs the deployment process by:

1. SSHing into the target EC2 instance.
2. Performing a hard reset to the latest pushed commit.
3. Reinstalling dependencies.
4. Restarting the backend application using **PM2** (`healthcare-api`).
5. Building the Angular frontend application.
6. Serving the frontend through **Nginx**.
7. Reverse-proxying all `/api` requests from Nginx to the backend service.
...
Custom slash commands in `.claude/commands/`:
   ...
   `/deploy-preflight` — mirrors the deploy pipeline locally before you push; GO/NO-GO verdict, verify-only.

   `/sprint-report` — sprint/changelog report from git history, by app area and contributor.
