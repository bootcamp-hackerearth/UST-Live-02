...
const getRecordFileName = (title: string, url: string) => {
  ...
  const extension = /\.([a-z0-9]+)$/i.exec(urlWithoutQuery)?.[1] ?? "pdf";
  const hasExtension = title
    .toLowerCase()
...
export default function HealthRecordScreen() {
  ...
  const renderHealthRecordItem = useCallback(
    (listItem: { item: HealthRecordListItem }) => {
      const healthRecordItem = listItem.item;

      if (healthRecordItem.type === "TIMELINE") {
        return (
          <View style={styles.listItem}>
            <TimelineCard consultation={healthRecordItem.value} />
          </View>
        );
      }

      if (healthRecordItem.type === "PRESCRIPTIONS") {
        return (
          <View style={styles.listItem}>
            <PrescriptionCard
              item={healthRecordItem.value}
      ...
      if (healthRecordItem.type === "REPORTS") {
        return (
          <View style={styles.listItem}>
            <LabReportCard
              report={healthRecordItem.value}
      ...
      return (
        <View style={styles.listItem}>
          <MedicalDocumentCard
            document={healthRecordItem.value}
