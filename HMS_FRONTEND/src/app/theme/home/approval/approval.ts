...
export class ApprovalComponent implements OnInit {
  ...
  private updateApprovalCounts(): void {
    this.approvalUiData.pendingCount = this.userData.filter((user) => user.status === 'Pending').length;
    this.approvalUiData.verifiedCount = this.userData.filter((user) => user.isVerified === true).length;
    this.approvalUiData.inActiveCount = this.userData.filter((user) => user.status === 'Inactive').length;
    this.approvalUiData.firstLoginCount = this.userData.filter((user) => user.firstLogin === true).length;
  }
