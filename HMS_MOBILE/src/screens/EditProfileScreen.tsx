...
const validateLettersField = (value: string, label: string) => {
...
const validateEmergencyContactName = (value: string) => {
  ...
  if (!isValidName(value))
    return "Use 2-50 letters, spaces, apostrophes or hyphens";
  return "";
};
...
export default function EditProfile() {
  ...
  const validateForm = () => {
    ...
    const cityError = validateLettersField(city, "City");
    if (cityError) newErrors.city = cityError;

    const stateError = validateLettersField(state, "State");
    if (stateError) newErrors.state = stateError;

    const countryError = validateLettersField(country, "Country");
    if (countryError) newErrors.country = countryError;

    const pincodeError = validatePincode(pincode);
    if (pincodeError) newErrors.pincode = pincodeError;

    const nameError = validateEmergencyContactName(emergencyContactName);
    if (nameError) newErrors.emergencyContactName = nameError;

    const phoneError = validateEmergencyPhone(emergencyContactPhone);
