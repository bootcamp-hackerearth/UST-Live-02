...
const getUserProfile = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: 'internal server error during getUserProfile'
        });
    }
}
...
const getPatientId = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: true,
            message: 'internal server error during getPatientId'
        });
    }
}
...
const getAvailableTimeSlots = async (req, res) => {
    try {
        ...
        if (inputDate <= today) return res.status(400).json({
            success: false,
            message: 'You cant book appointment in past'
        });
        ...
        if (!allSlots) return res.status(404).json({
            success: false,
            message: 'No slots found for doctor'
        });
        ...
        return res.status(500).json({
            success: false,
            message: 'internal server error during get available time slots'
        });
    }
}
