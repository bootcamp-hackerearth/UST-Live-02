...
Run the same gates the deploy workflow runs on the server, **locally, before a push**, so a broken build or a backend that won't boot never reaches EC2. This is **verify-only**: end with a GO / NO-GO verdict — never fix, commit, push, deploy, or edit tracked files.
