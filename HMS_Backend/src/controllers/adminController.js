...
const formatEmployeeList = require("../utils/employeeResponseBuilder");
const formatEmployeeProfile = require("../utils/employeeProfileBuilder");
const applyEmployeeUpdates = require("../utils/employeeUpdateBuilder");
const writeAuditLog = require("../utils/auditLogger");
const resolveAuditActor = require("../utils/actorResolver");
const removeEmployeeAccount = require("../utils/employeeAccountRemover");
const cancelAppointmentsForDoctor = require("../utils/doctorAppointmentCanceller");
const createEmployeeAccount = require("../utils/employeeAccountCreator");
const parsePaginationOptions = require("../utils/paginationParser");
...
const getEmployeesByStatus = async (status, res) => {
  ...
  const formattedEmployees = formatEmployeeList(employees, users);
...
exports.createEmployee = async (req, res) => {
  ...
  try {
    ...
    const { employee, user } = await createEmployeeAccount(req, {
      roles: ["STAFF"],
...
exports.getEmployee = async (req, res) => {
  try {
    ...
    const profile = formatEmployeeProfile(employee);
...
exports.rejectEmployee = async (req, res) => {
  try {
    ...
    await removeEmployeeAccount(employeeCode);
...
exports.deleteEmployee = async (req, res) => {
  ...
  try {
    ...
    await cancelAppointmentsForDoctor(employeeCode, employee.name, actor);
    await removeEmployeeAccount(employeeCode);
...
exports.getAuditLogs = async (req, res) => {
  try {
    const { page, limit, skip } = parsePaginationOptions(req.query, 20);
