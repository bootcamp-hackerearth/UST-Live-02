...
const getDashboardData = async (req, res) => {
    try {
        const [employeeCount, activeCount, pendingApprovalCount, pendingVerifyCount, patientCount, appointmentCount, departmentCount]
            = await Promise.all([
        ...
        if (medicalRoles.has(existingUser?.role)) {
            if (!data.medicalRegistrationNo) {
                return res.status(422).json({
                    success: false,
                    message: "This role requires medical registration number"
                });
            }
            if (!data.qualification) {
                return res.status(422).json({
                    success: false,
                    message: "This role requires qualification"
                });
            }
        }
