...
export class AppointmentBookComponent
  ...
  private readonly selectedDoctorId = signal<string>('');

  // Scrollable date strip: next STRIP_DAYS days with availability context.
  readonly stripDays = computed<DayOption[]>(() => {
    const minIso = this.minDate();
    const minMs = new Date(minIso + 'T00:00:00').getTime();
    ...
    const availableDays = doctor
      ? new Set((doctor.availabilitySlots || []).map((s: AvailabilitySlot) => s.day as string))
      : null;
    ...
    for (let i = 0; i < STRIP_DAYS; i++) {
      ...
      const isPast = d.getTime() < minMs;
      const weekday = DAY_MAP[d.getDay()];
      const isUnavailable = !isPast && availableDays !== null && !availableDays.has(weekday);

      result.push({
        iso,
        dayName: i === 0 ? 'Today' : i === 1 ? 'Tomorrow' : DAY_ABBR[d.getDay()],
  ...
  selectDate(iso: string): void {
    this.form.get('appointmentDate')!.setValue(iso);
