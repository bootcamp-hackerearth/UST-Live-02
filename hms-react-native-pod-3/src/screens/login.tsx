...
export default function LoginScreen() {
  ...
  const validateForm = () => {
    ...
    const ERRORS = {
      PASSWORD_REQUIRED: "Password is required", //NOSONAR
      PASSWORD_MIN_LENGTH: "Minimum 8 characters required" //NOSONAR
    };


    if (!password) {
      error.password = ERRORS.PASSWORD_REQUIRED;
    } else if (password.length < 8) {
      error.password = ERRORS.PASSWORD_MIN_LENGTH;
    }
    setErrors(error);
